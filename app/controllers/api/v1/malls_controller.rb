  ...
  module V1
    class MallsController < ApplicationController
      ...
      def index
        ...
        render json: {status: 'SUCCESS',message:'Loaded malls', data:malls},status: 200
      end
      def show
        mall = Mall.find(params[:id])
        render json: {status: 'SUCCESS',message:'Loaded mall', data:mall},status: 200
      end
      def create
        mall = Mall.new(mall_params)
        if mall.save
          render json: {status: 'SUCCESS',message:'Saved mall', data:mall},status: 201
        else
      ...
      def update
        ...
        if mall.update_attributes(mall_params)
          render json: {status: 'SUCCESS',message:'Updated mall', data:mall},status: 200
        else
      ...
      def destroy
        mall = Mall.find(params[:id])
        mall.destroy
        render json: {status: 'SUCCESS',message:'Deleted mall', data:mall},status: 200
      end
      private
      def mall_params
